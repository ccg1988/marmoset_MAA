...
x_deg=[37.5;45;90;180];
fitX=nan(N, 1200);
fitY=nan(N, 2);
fitValues=nan(N, 1200);
fitPoints=nan(N, length(x_deg));
fitParas=nan(N,4);
cond=6; % azi_Gau_front/RSS_front/Gau_rear***ele_Gau_2to32/4to26/4to12kHz
result_save=cell(N,1);
for n = 1 : N
nCorrect=T{n+(cond-1)*N, 5:12}'; nCorrect(1:4)=[];
total=T{n+(cond-1)*N, 13:20}';  total(1:4)=[];
...
figure
CR=nan(N,1);
CR(1)=T{1+(cond-1)*N, 3}; CR(2)=112; CR(3)=124; 
for n = 1 : N
...
plot([0 x_deg(end)],[0.5 0.5],"LineStyle","-.",'LineWidth',1,'Color','k')
...
xlim([0 x_deg(end)]); xlabel('Angle (\circ)'); ylim([0 1]); ylabel('Corrected hit rates');
xticks(0:7.5:x_deg(end))
%%
figure
for n = 1 : N
...
lgd= legend({'',['M3T MAA=',num2str(round(fitParas(1,1),2)),...
    ...
    '\circ'],''}); 
lgd.Box='off'; lgd.FontSize=12; lgd.Location='southeast';
xlim([0 x_deg(end)]); xlabel('Angle (\circ)'); ylabel('Percent correct');
xticks(0:7.5:x_deg(end))
%%
W = 17.4 ; %centimeter
H = 5 ; %centimeter
