function [thres_y, xAll, fitValuesAll] = plotPsych(result,plotOptions)
...
xAll=[xLow, x, xHigh]; fitValuesAll=[fitValuesLow, fitValues, fitValuesHigh];
...
if plotOptions.plotThresh
    if result.options.logspace
        ...
        thres_y=[ymin,result.Fit(4)+(1-result.Fit(3)-result.Fit(4)).*result.options.threshPC];
        plot([result.Fit(1),result.Fit(1)],thres_y,'-','Color',plotOptions.lineColor);
    end
end
