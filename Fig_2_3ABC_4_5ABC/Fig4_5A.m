...
x_deg=[7.5;15;22.5;30;37.5;45;90;180];
fitX=nan(N, 1200);
fitY=nan(N, 2);
fitValues=nan(N, 1200);
fitPoints=nan(N, length(x_deg));
fitParas=nan(N,4);
cond=4; % azi_Gau_front/RSS_front/Gau_rear***ele_Gau_2to32/4to26/4to12kHz
result_save=cell(N,1);
for n = 1 : N
nCorrect=T{n+(cond-1)*N, 5:12}'; 
total=T{n+(cond-1)*N, 13:20}';  
...
if n==2 %M94W missed 1st value
data(1,:)=[]; fitPoints(n,1)=nan;
end 
